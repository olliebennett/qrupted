...
(function ($) {
	...
	var QRCode = function (text, level, version, quiet) {
			...
			this.getModules = qr.getModules;
			this.setModules = qr.setModules;
		...
		setOverlay = function (originalDataArray, settings) {

			var newDataArray = originalDataArray;

			var word = settings.textOverlay.split("");
			//console.log(word);

			var transparentOverlay = settings.textOverlayTransparency;
			//console.log("Transparent overlay? : " + transparentOverlay);

			var showBorder = settings.textOverlayBorder;
			...
			for ( var word_cursor = 0; word_cursor < word.length; word_cursor++ ) {
				...
				//console.log("Total length of ASCII word: " + word_ASCII_length);

			}

			var start_x = Math.floor(size / 2) - Math.ceil(word_ASCII_length / 2) + 1;
			var start_y = Math.floor(size / 2) - 4;
			// store location to start drawing letter from.
			...
			for ( var word_cursor = 0; word_cursor < word.length; word_cursor++ ) {
				...
				for (var pixel_x = 0; pixel_x < letter_ascii[0].length; pixel_x ++) {
					for (var pixel_y= 0; pixel_y < letter_ascii.length; pixel_y ++) {
						var c = letter_ascii[pixel_y][pixel_x];
						//console.log("Processing pixels at [" + pixel_y + "," + pixel_x + "] => '" + c + "'.");
						//console.log("Current state: " + newDataArray[cursor_y + pixel_y][cursor_x + pixel_x]);
						if (c == "X") {
							newDataArray[cursor_y + pixel_y][cursor_x + pixel_x] = true;
						} else if (c == "0" || (c == "-" && transparentOverlay)) {
							newDataArray[cursor_y + pixel_y][cursor_x + pixel_x] = false;
						} else if (c == "-") {
							// no change needed
						} else {
							showError("Unknown/invalid ascii point - '" + c + "' - must be '0' (white), 'X' (black), or '-' (transparent).");
						}
					}
				}

				// Update cursor (move to right)
				cursor_x += letter_ascii[0].length - 1;
		...
		drawOnCanvas = function (canvas, settings) {
			...
			if (!qr) {
				showError("Unknown problem while creating QR Code");
				return null;
			}

			// Perform the overlay of text
			qr.setModules(setOverlay(qr.getModules(), settings));
		...
		defaults = {
			...
			// content
			text: 'Enter the text to store in the QR code',
	...
	var qrcode = function() {
		...
		var qrcode = function(typeNumber, errorCorrectLevel) {
			...
			_this.setModules = function(modules_new) {
				_modules = modules_new;
			}
